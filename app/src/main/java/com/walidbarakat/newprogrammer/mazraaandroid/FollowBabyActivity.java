...
public class FollowBabyActivity extends AppCompatActivity {
    public static int BTahsynNotificationId, BMedicinNotificationId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnaddall.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    data.clear();
                    Cursor Cr = db.mDatabase.rawQuery("select distinct(babynum) from allnames where babynum<>'' and mothernum is null", null);
                    Cr.moveToFirst();
                    while (!Cr.isAfterLast()) {
                        String id = Cr.getString(Cr.getColumnIndex("BabyNum"));
                        data.add(id);
                        Cr.moveToNext();
                    }
                    ArrayAdapter NoCoreAdapter = new ArrayAdapter(FollowBabyActivity.this, android.R.layout.simple_list_item_1, data);
                    ...
                    finaladapter = new ArrayAdapter<>(FollowBabyActivity.this,
        ...
        btnaddmedicin.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(FollowBabyActivity.this);
                builder.setTitle("ادخل اسم العلاج");

                final EditText input = new EditText(FollowBabyActivity.this);
                ...
                builder.setPositiveButton("اضافه", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        ArrayAdapter<String> adapter = new ArrayAdapter<String>(FollowBabyActivity.this,
...
// Specify the type of input expected; this, for example, sets the input as a password, and will mask the text
        ...
        btnaddone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ls.add(valu2);
                ArrayAdapter<Integer> adapter2 = new ArrayAdapter<Integer>(FollowBabyActivity.this,
                        android.R.layout.simple_list_item_1, ls);
    ...
    public void Clear() {
        try {
            ...
            ArrayList<String> data = new ArrayList<>();
            data.clear();
            Cursor Cr = db.mDatabase.rawQuery("select distinct(tahsynb) from allnames where tahsynb<>'' and babynum<>'' and TahsynB<>'TahsynB'", null);
            Cr.moveToFirst();
            while (!Cr.isAfterLast()) {
                String id = Cr.getString(Cr.getColumnIndex("TahsynB"));
                data.add(id);
                Cr.moveToNext();
            }
            data.add("لا شىء");
            ...
            data2.clear();
            Cr = db.mDatabase.rawQuery("select distinct(Medicinb) from allnames where Medicinb<>'' and babynum<>'' and MedicinB<>'MedicinB'", null);
            Cr.moveToFirst();
            while (!Cr.isAfterLast()) {
                String id = Cr.getString(Cr.getColumnIndex("MedicinB"));
                data2.add(id);
                Cr.moveToNext();
            }
            data2.add("لا شىء");
            ...
            data3.clear();
            Cr = db.mDatabase.rawQuery("select distinct(babynum) from allnames where babynum<>'' and mothernum is null", null);
            Cr.moveToFirst();
            while (!Cr.isAfterLast()) {
                String id = Cr.getString(Cr.getColumnIndex("BabyNum"));
                data3.add(id);
    ...
    public void BabySrchResult() {

        db.openDatabase();
        Cursor Cr = db.mDatabase.rawQuery("select * from allnames where FollowAutonum='" + getIntent().getStringExtra("BabySrchNum") + "'", null);
        try {
            Cr.moveToFirst();
            ...
            if (Count > 0) {
                ...
                String valu2 = Cr.getString(Cr.getColumnIndex("MedicinB"));
                for (int a = 0; a < spnrmedicin.getAdapter().getCount(); a++) {
                    if (spnrmedicin.getItemAtPosition(a).equals(valu2)) {
                        spnrmedicin.setSelection(a);
                        break;
                ...
                String valu1 = Cr.getString(Cr.getColumnIndex("TahsynB"));
                for (int a = 0; a < spnrtahsyn.getAdapter().getCount(); a++) {
                    if (spnrtahsyn.getItemAtPosition(a).equals(valu1)) {
                        spnrtahsyn.setSelection(a);
                        break;
                ...
                //--------------------------------------------------
                //DtpMedicinDate.Value = Dr("MedicinDateB")
                txtshowdate.setText(String.valueOf(Cr.getString(Cr.getColumnIndex("MedicinDateB"))));
                ...
                //--------------------------------------------------
                //DtpTahsynDate.Value = Dr("TahsynDateB")
                txtshowdate.setText(String.valueOf(Cr.getString(Cr.getColumnIndex("TahsynDateB"))));
                ...
                //--------------------------------------------------------
                txtnotes.setText(Cr.getString(Cr.getColumnIndex("NotesB")));
                Cr = db.mDatabase.rawQuery("select * from allnames where FollowAutonum='" + getIntent().getStringExtra("BabySrchNum") + "'", null);
                try {
                    Cr.moveToFirst();
                    ArrayList<String> data = new ArrayList<>();
                    data.clear();
                    while (!Cr.isAfterLast()) {
                        String id2 = Cr.getString(Cr.getColumnIndex("BabyNum"));
                        data.add(id2);
                        Cr.moveToNext();
                    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.BtnNew) {
            ...
            Intent intent = new Intent(FollowBabyActivity.this, BabySrch.class);
            ...
            if (lblcount1.getText().toString().trim().equals("0")) {
                Toast.makeText(this, "من فضلك اختار المواليد", Toast.LENGTH_SHORT).show();
                return super.onOptionsItemSelected(item);
            }
            try {
                ...
                if (Count > 0) {
                    ...
                    for (int i = 0; i < lstchoose.getAdapter().getCount(); i++) {
                        String Tahsyn, Medicin, NotesM, Username, BabyNum, MedicinDate, TahsynDate, Gender, Birthdate, FollowAutonum;
                        Username = MainActivity.GeneralUser.trim();
                        Tahsyn = CheckTahsyn;
                        BabyNum = lstchoose.getItemAtPosition(i).toString().trim();
                        Medicin = CheckMedicin;
                        NotesM = txtnotes.getText().toString().trim();
                        FollowAutonum = txtcode.getText().toString().trim();
                        Cr = db.mDatabase.rawQuery("select * from allnames where babynum='" + lstchoose.getItemAtPosition(i).toString().trim() + "'", null);
                        Cr.moveToFirst();
                        Count = Cr.getCount();
                        if (Count > 0) {
                            Gender = Cr.getString(Cr.getColumnIndex("Gender"));

                            txttestdate.setText(String.valueOf(Cr.getString(Cr.getColumnIndex("BirthDate"))));
                            String mysourcestring = txttestdate.getText().toString();
                            String year = mysourcestring.substring(0, 4);
                            String month = mysourcestring.substring(5, 7);
                            String day = mysourcestring.substring(8, 10);
                            String Date1, Date2;
                            ...
                            if (Integer.parseInt(hour) > 12) {
                                int total = Integer.parseInt(hour) - 12;
                                if (String.valueOf(total).length() == 1) {
                                    Date2 = "0" + total + ":" + minute + " " + "PM";
                                } else {
                                    Date2 = total + ":" + minute + " " + "PM";
                                }
                            } else {
                                Date2 = hour + ":" + minute + " " + "AM";
                            }
                            //---------------------------------------------------------
                            String string_date = Date1 + " " + Date2;
                        ...
                        db.Insert_to_AllnamesB(Username, Tahsyn, BabyNum, Medicin, NotesM, TahsynDate, Gender, FollowAutonum, Birthdate, MedicinDate);
                    ...
                    if (chkremember.isChecked() == true) {
                        ...
                        for (int i = 0; i < lstchoose.getAdapter().getCount(); i++) {
                            String Tahsyn, Medicin, NotesM, BabyNum, MedicinDate, TahsynDate, Gender, Birthdate;
                            Tahsyn = CheckTahsyn;
                            BabyNum = lstchoose.getItemAtPosition(i).toString().trim();
                            Medicin = CheckMedicin;
                            NotesM = txtnotes.getText().toString().trim();
                            ...
                            db.Insert_to_BabyRemember(Tahsyn, BabyNum, Medicin, NotesM, TahsynDate, Gender, Birthdate, MedicinDate);
                            Cr.close();
                        }
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    try {
                        ...
                        if (Count > 0) {
                            ...
                            Toast.makeText(FollowBabyActivity.this, "تم الحذف بنجاح", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(FollowBabyActivity.this, "لا يوجد بيانات لحذفها", Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception g) {
                        Log.e("", g.getMessage());
                    }
...
//---------------------------------------------------------------------------------------------
            ...
            Date d2 = f2.parse(string_date2);
            milliseconds2 = d2.getTime();
            BTahsynNotificationId = (int) (long) milliseconds2;
            ...
            Intent notifyIntent = new Intent(FollowBabyActivity.this, BTahsynReceiver.class);
            PendingIntent pendingIntent = PendingIntent.getBroadcast
                    (FollowBabyActivity.this, BTahsynNotificationId, notifyIntent, PendingIntent.FLAG_UPDATE_CURRENT);
...
//---------------------------------------------------------------------------------------------
            ...
            Date d2 = f2.parse(string_date2);
            milliseconds2 = d2.getTime();
            BMedicinNotificationId = (int) (long) milliseconds2;
            ...
            Intent notifyIntent = new Intent(FollowBabyActivity.this, BMedicinReceiver.class);
            PendingIntent pendingIntent = PendingIntent.getBroadcast
                    (FollowBabyActivity.this, BMedicinNotificationId, notifyIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            AlarmManager alarmManager = (AlarmManager) getSystemService(Context.ALARM_SERVICE);
            alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(),
                    AlarmManager.INTERVAL_DAY, pendingIntent);
            //AlarmManager.INTERVAL_DAY,
